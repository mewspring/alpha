...
struct
	...
	fun aStar ( graph', start as (sx,sy) : (int * int), goal as (ex, ey))  =
		let
			val graph = Graph.copy(graph');
			val openList = BinoHeap.insert( BinoHeap.empty, 0,start );
			fun getParent(x,y) = case (Graph.at graph) (x, y) of (SOME (Graph.Node(_, _, (_,_,_, parent )))) => parent
			...
			|	pathfind' ( openList ) =
				let
					...
					fun getF(adjX, adjY) = case (Graph.at graph) (adjX, adjY) of (SOME (Graph.Node(_, _, (_,g,h, _ )))) => g+h

					fun doStuff (adjX, adjY) =
						let
							val (xy, adjl, adjC,adjG,adjH,adjParent) = case (Graph.at graph) (adjX, adjY) of (SOME (Graph.Node(xy, adjl, (c,g,h, parent )))) => (xy, adjl, c,g,h,parent)
						in
							if adjC = Black then
								0
							else if adjC = White then
								((Graph.update graph) ((adjX, adjY), (SOME (Graph.Node(xy, adjl, (Gray,calculateG(currentNode, (adjX, adjY)),calculateH(adjX, adjY), currentNode ))))); 1)
							else (*Gray*)
								let
								...
								in
									if newG >= adjG then
										0
									else ((Graph.update graph) ((adjX, adjY), (SOME (Graph.Node(xy, adjl, (Gray,newG,adjH, currentNode ))))); 2)
								end
						end

					val (_, adjList) = case ((Graph.at graph) (x, y)) of (SOME (Graph.Node(tXY, tadjList, (_, g, h, parent)))) => ((Graph.update graph) ((x, y), (SOME (Graph.Node(tXY, tadjList, (Black, g, h, parent))))),tadjList)
				in
	...
	fun dijkstra ( graph', start as (sx,sy) : (int * int), goal as (ex, ey)) =
	let
		val graph = Graph.copy(graph');
		...
		fun rewind (pos as (x,y)) =
			let
				val (value, adjXY, adjList) = case (Graph.at graph) (x, y) of (SOME (Graph.Node(_, adjXY::adjList, SOME value))) => (value, adjXY, adjList)
